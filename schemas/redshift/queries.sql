...
SELECT
    ...
    tenant_name,
    entity_type,
    -- Extract region from denormalized column
    attribute_category,
    ...
    PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY value_decimal) AS median_temperature,
    PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY value_decimal) AS p95_temperature
FROM fact_telemetry
WHERE ingested_at >= DATEADD(month, -6, GETDATE())
...
SELECT
    ...
    SUM(CASE WHEN value_int IS NOT NULL THEN 1 ELSE 0 END) AS int_count,
    SUM(CASE WHEN value_decimal IS NOT NULL THEN 1 ELSE 0 END) AS decimal_count,
    SUM(CASE WHEN value_bool IS NOT NULL THEN 1 ELSE 0 END) AS bool_count,
    SUM(CASE WHEN value_timestamp IS NOT NULL THEN 1 ELSE 0 END) AS timestamp_count,
    SUM(CASE WHEN value IS NOT NULL THEN 1 ELSE 0 END) AS text_count
FROM fact_telemetry
WHERE ingested_date >= DATEADD(day, -90, GETDATE())
GROUP BY 1, 2, 3
HAVING total_events > 1000  -- Filter low-volume attributes
...
status_durations AS (
    SELECT
        ...
        entity_type,
        status,
        DATEDIFF(second, prev_timestamp, ingested_at) AS duration_seconds
    FROM status_events
...
SELECT
    ...
    SUM(CASE WHEN status = 'online' THEN duration_seconds ELSE 0 END) AS online_seconds,
    SUM(CASE WHEN status = 'offline' THEN duration_seconds ELSE 0 END) AS offline_seconds,
    SUM(CASE WHEN status = 'warning' THEN duration_seconds ELSE 0 END) AS warning_seconds,
    SUM(duration_seconds) AS total_seconds,
    -- Uptime percentage
    ROUND(
        (SUM(CASE WHEN status = 'online' THEN duration_seconds ELSE 0 END)::DECIMAL 
         / NULLIF(SUM(duration_seconds), 0)) * 100, 
        2
    ) AS uptime_percent,
    -- Downtime events
    COUNT(CASE WHEN status = 'offline' THEN 1 END) AS downtime_events
...
SELECT
    ...
    ft.tenant_name,
    ft.entity_type,
    -- Current state from fact_current_state
    cs.status,
    cs.priority,
    cs.region,
    cs.environment,
    -- Alert metrics from time-series
    ...
    DATEDIFF(minute, MIN(ft.ingested_at), MAX(ft.ingested_at)) AS alert_duration_minutes
FROM fact_telemetry ft
JOIN fact_current_state cs ON cs.entity_id = ft.entity_id
WHERE ft.attribute_name = 'alert_generated'
...
SELECT
    ...
    -- Health percentages
    ROUND((SUM(CASE WHEN status = 'online' THEN 1 ELSE 0 END)::DECIMAL / COUNT(*)) * 100, 1) AS online_percent,
    -- Alert metrics
...
SELECT
    ...
    MODE(value_decimal) AS mode_value,
    -- Spread
    STDDEV(value_decimal) AS stddev_value,
    VARIANCE(value_decimal) AS variance_value,
    -- Range
    MIN(value_decimal) AS min_value,
    MAX(value_decimal) AS max_value,
    MAX(value_decimal) - MIN(value_decimal) AS range_value,
    -- Percentiles
    PERCENTILE_CONT(0.01) WITHIN GROUP (ORDER BY value_decimal) AS p01,
    PERCENTILE_CONT(0.05) WITHIN GROUP (ORDER BY value_decimal) AS p05,
    PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY value_decimal) AS p25,
    PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY value_decimal) AS p50,  -- median
    PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY value_decimal) AS p75,
    PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY value_decimal) AS p95,
    PERCENTILE_CONT(0.99) WITHIN GROUP (ORDER BY value_decimal) AS p99
FROM fact_telemetry
WHERE ingested_at >= DATEADD(day, -7, GETDATE())
  AND tenant_id = 123
  AND value_decimal IS NOT NULL
...
WITH attribute_pairs AS (
    ...
    FROM fact_telemetry a
    JOIN fact_telemetry b 
        ON b.entity_id = a.entity_id
        AND b.tenant_id = a.tenant_id
        AND ABS(DATEDIFF(second, a.ingested_at, b.ingested_at)) < 60  -- Within 1 minute
    WHERE a.ingested_at >= DATEADD(day, -7, GETDATE())
      AND a.tenant_id = 123
      AND a.attribute_name IN ('cpu_percent', 'memory_percent', 'disk_iops')
      AND b.attribute_name IN ('cpu_percent', 'memory_percent', 'disk_iops')
...
SELECT
    ...
    attr2_name,
    COUNT(*) AS pair_count,
    -- Correlation coefficient (simplified Pearson)
    CORR(attr1_value, attr2_value) AS correlation,
    -- Covariance
    COVAR_POP(attr1_value, attr2_value) AS covariance,
    -- Regression (slope)
    REGR_SLOPE(attr2_value, attr1_value) AS regression_slope,
    -- Regression (intercept)
    REGR_INTERCEPT(attr2_value, attr1_value) AS regression_intercept,
    -- R-squared
    POWER(CORR(attr1_value, attr2_value), 2) AS r_squared
...
WITH hourly_metrics AS (
    ...
    FROM fact_telemetry
    WHERE ingested_at >= DATEADD(day, -30, GETDATE())
      AND tenant_id = 123
...
SELECT
    ...
    -- Weekly seasonality (same hour, 7 days ago)
...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN entity_type = 'sensor' THEN entity_id END) AS sensor_count,
    COUNT(DISTINCT CASE WHEN entity_type = 'gateway' THEN entity_id END) AS gateway_count,
    -- Event metrics
    COUNT(*) AS total_events,
    COUNT(*) / 30.0 AS avg_events_per_day,
    COUNT(*) / (30.0 * 24.0) AS avg_events_per_hour,
    -- Attribute usage
    COUNT(DISTINCT attribute_id) AS distinct_attributes_used,
    -- Data volume (estimated)
    SUM(LENGTH(value)) AS total_text_bytes,
    SUM(LENGTH(value)) / 1024.0 / 1024.0 AS total_text_mb,
    -- Time range
    MIN(ingested_at) AS earliest_event,
    MAX(ingested_at) AS latest_event,
    DATEDIFF(hour, MIN(ingested_at), MAX(ingested_at)) AS active_hours
FROM fact_telemetry
WHERE ingested_at >= DATEADD(day, -30, GETDATE())
GROUP BY 1, 2
...
WITH daily_stats AS (
    ...
    FROM fact_telemetry
    WHERE ingested_at >= DATEADD(day, -90, GETDATE())
    GROUP BY 1, 2, 3, 4
)
SELECT
    tenant_id,
    ...
    entity_type,
    -- Current month
    AVG(CASE WHEN ingested_date >= DATE_TRUNC('month', GETDATE()) 
        THEN active_entities END) AS current_month_avg_entities,
    -- Previous month
    AVG(CASE WHEN ingested_date >= DATE_TRUNC('month', DATEADD(month, -1, GETDATE()))
            AND ingested_date < DATE_TRUNC('month', GETDATE())
        THEN active_entities END) AS prev_month_avg_entities,
    -- Month-over-month growth
    ROUND(
        ((AVG(CASE WHEN ingested_date >= DATE_TRUNC('month', GETDATE()) 
            THEN active_entities END) /
          NULLIF(AVG(CASE WHEN ingested_date >= DATE_TRUNC('month', DATEADD(month, -1, GETDATE()))
                    AND ingested_date < DATE_TRUNC('month', GETDATE())
                THEN active_entities END), 0)) - 1) * 100,
        2
    ) AS mom_growth_percent,
    -- Event volume growth
    SUM(CASE WHEN ingested_date >= DATE_TRUNC('month', GETDATE()) 
        THEN daily_events END) AS current_month_events,
    SUM(CASE WHEN ingested_date >= DATE_TRUNC('month', DATEADD(month, -1, GETDATE()))
            AND ingested_date < DATE_TRUNC('month', GETDATE())
        THEN daily_events END) AS prev_month_events
FROM daily_stats
GROUP BY 1, 2, 3
HAVING current_month_avg_entities IS NOT NULL
...
WITH tenant_metrics AS (
    SELECT
        ...
        COUNT(*) AS event_count,
        COUNT(DISTINCT entity_id) AS entity_count,
        COUNT(DISTINCT attribute_id) AS attribute_count,
        -- Estimate storage (rough calculation)
        SUM(
            LENGTH(COALESCE(value, '')) +
            COALESCE(8, 0) +  -- value_int (8 bytes if not null)
            COALESCE(8, 0) +  -- value_decimal (8 bytes if not null)
            COALESCE(1, 0)    -- value_bool (1 byte if not null)
        ) AS estimated_bytes
    FROM fact_telemetry
    WHERE ingested_at >= DATEADD(month, -3, GETDATE())
    GROUP BY 1, 2, 3
)
SELECT
    tenant_id,
    ...
    (estimated_bytes / 1024.0 / 1024.0 / 1024.0) * 0.023 AS storage_cost_usd,  -- $0.023/GB Redshift
    -- Processing costs (estimated)
    event_count * 0.000001 AS processing_cost_usd,  -- $1 per million events
    -- Total estimated cost
    ((estimated_bytes / 1024.0 / 1024.0 / 1024.0) * 0.023) + 
...
WHERE hour >= DATEADD(hour, -24, GETDATE())
  AND tenant_id = 123
  AND attribute_name IN ('cpu_percent', 'memory_percent', 'temperature_celsius')
...
SELECT
    ...
    DATEDIFF(second, MAX(ingested_at), GETDATE()) AS lag_seconds,
    DATEDIFF(minute, MAX(ingested_at), GETDATE()) AS lag_minutes,
    COUNT(*) AS events_last_hour,
    COUNT(DISTINCT entity_id) AS active_entities_last_hour,
    -- Freshness status
    CASE
        WHEN DATEDIFF(minute, MAX(ingested_at), GETDATE()) < 5 THEN 'healthy'
        WHEN DATEDIFF(minute, MAX(ingested_at), GETDATE()) < 15 THEN 'degraded'
        ELSE 'stale'
    END AS freshness_status
FROM fact_telemetry
WHERE ingested_at >= DATEADD(hour, -1, GETDATE())
GROUP BY 1, 2
