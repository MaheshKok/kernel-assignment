...
CREATE INDEX idx_attributes_hot ON attributes(attribute_id)
WHERE
    is_hot = TRUE;
...
FROM
    entities e
    JOIN entity_jsonb ej ON e.entity_id = ej.entity_id
...
numeric_aggregates AS (
    ...
    FROM
        ...
        JOIN entity_values_ts ev ON ce.entity_id = ev.entity_id
            AND ce.tenant_id = ev.tenant_id
        JOIN attributes a ON ev.attribute_id = a.attribute_id
    WHERE
        ev.tenant_id = 123
